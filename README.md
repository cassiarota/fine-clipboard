...
到 [Releases](https://github.com/cassiarota/win-paste/releases) 下载最新的
`PasteNowWin-*-win-x64.zip`,解压后双击 `PasteNowWin.exe` 即可运行——**自带 .NET 运行时,
无需额外安装**。程序常驻系统托盘(右下角橙色图标)。
...
- 历史去重(相同内容自动置顶,不重复入库)
- 弹窗内:`↑↓` 选择 · `Enter` 粘贴 · `Ctrl+Enter` 纯文本粘贴 · `Ctrl+P` 置顶 · `Delete` 删除 · `Esc` 关闭
...
- **MVP 未实现**(见计划表 P2–P4):标签栏分组、多屏精确定位、自定义快捷键、排除规则、云同步、安装器。
- 粘贴注入无法作用于**以管理员权限运行**的窗口(本程序是 asInvoker)。如需对管理员窗口粘贴,
  需让本程序也以管理员身份运行。
- 多显示器下弹窗目前固定居中**主屏**(用 `SystemParameters.WorkArea`),P2 会改为跟随光标所在屏幕。
- `Ctrl+Shift+V` 为全局热键,会覆盖个别应用内置的「无格式粘贴」;后续做成可配置。
...
| 阶段 | 模块 | 交付物 | 验证标准 |
|---|---|---|---|
| **P1 MVP** ✅ | 监听 / 热键 / 存储 / 粘贴 / 弹窗 / 托盘 | 可构建运行的剪贴板管理器 | 复制→入库→`Ctrl+Shift+V`选择→粘贴;`Ctrl+Shift+B`纯文本;文本/图片/文件 |
| P2 体验 | 置顶 UI、删除、标签分组、多屏定位、高 DPI | 右键菜单 + 标签栏 | 置顶常驻顶部;高分屏定位准确 |
| P3 配置 | 自定义快捷键、排除规则、声音 | 完善设置页 | 快捷键可改并持久化 |
| P4 打包 | 单文件发布 + 安装器 + 自动更新 | `.exe` / MSIX | 双击安装,开机自启 |
